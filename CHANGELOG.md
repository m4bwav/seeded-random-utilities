...
- An unknown algorithm name throws `TypeError`; 1.1.4 silently used `Math.random()`. `null` as the algorithm now means sfc32 (1.1.4: `Math.random()`). A seed that is not a string, a finite number or an object with `next()` throws `TypeError`.
...
- `getRandomInteger`, `getRandomIntegerInclusive` and `getRandomFloat` take `(min, max)`, or `(max)`. They give the same numbers as the 1.1.4 methods, and throw `RangeError` for an empty or reversed range.
...
- `getUniqueRandomIntegers(amount, max)` and `getUniqueRandomIntegers(amount, min, max)`, which take time and memory in proportion to the amount, not the range.
